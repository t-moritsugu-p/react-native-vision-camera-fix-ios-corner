...
final class HybridBarcodeScannerOutput: HybridCameraOutputSpec, NativeCameraOutput {
  ...
  private func scan(_ buffer: CMSampleBuffer) {
    ...
    // Barcode CameraOutput buffers stay in sensor order. Preview mirroring is
    // applied by VisionCamera's Preview View, so camera points must not be
    // mirrored here or the front-camera transform would be applied twice.
